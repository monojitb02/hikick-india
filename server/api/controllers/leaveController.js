...
module.exports = {
    ...
    applyLeave: function(req, res) {
        ...
        companyUtil.knowCompletedAttendanceDate(req.sender.companyProfile.company)
            .then(function(companyData) {
                var completedAttendanceDate = companyData.attendanceCompletedUpto,
                    aditionalLeave = 0;
                Q.all([
                        leaveUtil.getApprovedApplications(req.sender._id, completedAttendanceDate),
                        leaveUtil.getLeaveAccount(req.sender._id)
                    ])
        ...
        var workflow = lib.workflow(req, res),
            ...
            getAllUsers = function(sender) {
                if (sender.companyProfile.role.trueName === 'admin') {
                    return userUtil.getAll(sender.companyProfile.company);
                } else {
                    return userUtil.getSubordinates([sender._id]);
                }
            };
        getAllUsers(req.sender)
            .then(function(userIds) {
                return leaveUtil.getManageableApplication(userIds, workflow);
            }, function(err) {
    ...
    viewAllLeaveAccount: function(req, res) {
        ...
        getAllUsers(req.sender)
            .then(function(users) {
                return leaveUtil.getAllLeaveAccount(users, req.sender._id);
            }, function(err) {
                var deferred = Q.defer();
                deferred.reject(err);
                return deferred.promise;
